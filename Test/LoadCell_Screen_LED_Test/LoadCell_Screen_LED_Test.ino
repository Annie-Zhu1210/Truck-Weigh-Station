...
const float WEIGHT_THRESHOLD = 300.0;  // 300g threshold
const unsigned long DELAY_TIME = 5000;  // 5 seconds in milliseconds

// State tracking variables
bool isAboveThreshold = false;
bool isServoAt0 = false;  // Now tracking if servo is at 0 degrees (closed position)
...
void setup() {
  ...
  myServo.write(90);  // Start at 90 degrees (open position)
  ...
  // IMPORTANT: Set your calibration value here after running calibration
  float calibrationValue = 1.0; // Replace with your actual value after calibration
  
  // Uncomment this to load calibration from EEPROM instead:
  ...
  // Stabilizing time for better accuracy
...
void loop() {
  ...
  if (newDataReady) {
    if (millis() > t + serialPrintInterval) {
      ...
      // Servo control logic with 5-second delay (REVERSED)
      if (weight > WEIGHT_THRESHOLD) {
        ...
        if (!isAboveThreshold) {
          ...
          // Already above threshold - check if 5 seconds have passed
          if (!isServoAt0 && (millis() - thresholdStartTime >= DELAY_TIME)) {
            // 5 seconds have passed, move servo to 0 degrees (closed)
            myServo.write(0);
            isServoAt0 = true;
        ...
        if (isServoAt0 && belowThresholdStartTime > 0 && 
          ...
          // 5 seconds below threshold, move servo back to 90 degrees (open)
          myServo.write(90);
          isServoAt0 = false;
          belowThresholdStartTime = 0;
      ...
      // Show message based on weight on second row
      lcd.setCursor(0, 1);
      if (weight > WEIGHT_THRESHOLD) {
